...
6. `complete_failure` 情境已不再直接送進 confirm；目前前端會留在 start 階段並顯示重拍 / 換圖提示，analysis 狀態也會保留在 `draft` 以支援重新上傳
7. `re-estimate` 第一版 API 與前端互動已開始落地，使用者可在 Candidate Confirmation 補充備註，例如「我只吃半份」或「這不是甜不辣，是炸雞」，再取得一版 AI 新建議並選擇是否套用
...
2. 再把辨識結果改成可表達狀態的結果物件，而不是只回傳候選清單，讓後續流程能分辨成功、部分成功與完全失敗
3. 接著讓圖片上傳回應能直接帶出狀態與提示訊息，避免前端只能靠候選是否為空來猜目前發生了什麼事
4. 最後由前端承接這些狀態：部分成功時進入 candidate review；完全失敗時直接顯示辨識失敗並引導重拍或換圖

目前這 4 層中的第一階段已開始落地，重點是先把可恢復失敗從一般 500 錯誤中分離出來，後續再把它收斂成「部分成功」與「完全失敗」兩種不同產品路徑。

補充說明：目前 `recognition_status`、`complete_failure` 重拍分流與 `re-estimate` 第一版已開始落地，但 `partial` 判定、建議對位與最小觀測性仍屬後續收斂項目，因此本文件第 4、5、6、8 節仍有一部分屬於目標方向，不應誤讀成全部完成。
