...
| 編號 | 主題 | 求職加分 | 技術依賴 |
|------|------|---------|---------|
| V2-01 | RAG + pgvector 向量營養查詢 | ★★★ | 需建立知識庫 |
| V2-02 | AI 事件追蹤（DB 可追溯 + Token 監控）| ★★★ | 與 V2-01 同步做 |
| V2-03 | 多模型切換架構 | ★★☆ | 架構已有基礎 |
| V2-04 | Prompt 版本管理 | ★★☆ | 極低，加 log 欄位 |
| V2-05 | 圖片前處理（送 API 前壓縮）| ★★☆ | 低，加一個處理步驟 |
| V2-06 | Token / Payload 壓縮 | ★☆☆ | 限縮為重新估算範圍 |
...
1. **V2-04 Prompt 版本管理** — 成本最低，先做，建立量測基礎
2. **V2-02 API 成本監控** — 配合 V2-01 RAG 需要，先建好
3. **V2-05 圖片前處理** — 簡單，可與上面一起做
4. **V2-01 RAG + pgvector** — 核心項目，需建立知識庫和 embedding pipeline
5. **V2-03 多模型切換** — 架構已有基礎，RAG 完成後做
