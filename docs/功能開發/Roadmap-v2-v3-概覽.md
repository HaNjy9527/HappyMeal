...
- 日期：2026-05-09
- 最後更新：2026-05-13
- 狀態：進行中（V2-01、V2-02、V2-04、V2-05 程式碼完成，V2-03 暫緩）
...
| V2-05 | 圖片前處理（送 API 前壓縮）           | ★★☆      | 低，加一個處理步驟 | ✅ 程式碼完成               |
...
- `recognition_openai.py` 已新增 `preprocess_image()`，會在送出前先處理圖片
- `get_image_data_url()` 已改為預設走前處理流程，而不是直接讀原始檔
- `config.py` 已加入 `IMAGE_PREPROCESS_ENABLED`、`IMAGE_PREPROCESS_MAX_PX` 設定
- 專案已加入 `Pillow` 依賴供圖片 resize / JPEG 壓縮使用
...
- 已確認辨識流程會呼叫前處理後的 `get_image_data_url()`
- 後端辨識測試 `tests/test_analysis_recognition.py` 目前全數通過
- 尚未補專門驗證 resize / 壓縮結果的獨立單元測試，可作為後續補強項目
...
4. ✅ **V2-05 圖片前處理** — `preprocess_image()`、設定開關與 JPEG 壓縮流程已完成
