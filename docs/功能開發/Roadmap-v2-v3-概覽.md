...
| V2-02 | AI 事件追蹤（DB 可追溯 + Token 監控）| ★★★ | 與 V2-01 同步做 | 待開發 |
| V2-03 | 多模型切換架構 | ★★☆ | 架構已有基礎 | 待開發 |
| V2-04 | Prompt 版本管理 | ★★☆ | 極低，加 log 欄位 | 待開發 |
...
1. ✅ **V2-01 RAG + pgvector** — 核心項目，已完成程式碼，需執行 import + generate 腳本驗收
2. **V2-02 AI 事件追蹤** — 配合 RAG 上線後一起追蹤 embedding API 成本，下一個執行
3. **V2-04 Prompt 版本管理** — 成本最低，可與 V2-02 同步
4. **V2-05 圖片前處理** — 簡單，可與上面一起做
5. **V2-03 多模型切換** — 架構已有基礎，RAG 驗收後做
