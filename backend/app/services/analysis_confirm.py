...
NUTRITION_PRESETS = {
    "chicken_salad": NutritionPreset("bowl", Decimal("320.00"), Decimal("28.00"), Decimal("18.00"), Decimal("12.00")),
    "boiled_egg": NutritionPreset("pcs", Decimal("78.00"), Decimal("6.50"), Decimal("5.30"), Decimal("0.60")),
    "grilled_chicken_rice": NutritionPreset("plate", Decimal("520.00"), Decimal("34.00"), Decimal("12.00"), Decimal("63.00")),
    "stir_fried_vegetables": NutritionPreset("plate", Decimal("160.00"), Decimal("5.00"), Decimal("7.00"), Decimal("18.00")),
    "grilled_salmon": NutritionPreset("fillet", Decimal("280.00"), Decimal("26.00"), Decimal("18.00"), Decimal("0.00")),
    "brown_rice": NutritionPreset("bowl", Decimal("216.00"), Decimal("5.00"), Decimal("1.80"), Decimal("45.00")),
    "steamed_broccoli": NutritionPreset("bowl", Decimal("55.00"), Decimal("3.70"), Decimal("0.60"), Decimal("11.00")),
}
...
def build_analysis_item(payload: AnalysisConfirmItemRequest) -> FoodAnalysisItem:
    preset = NUTRITION_PRESETS.get(payload.normalized_food_name)
    if preset is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Unsupported food item")

    if payload.portion_unit != preset.portion_unit:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Portion unit does not match the food item")

    multiplier = payload.portion_value
    return FoodAnalysisItem(
        ...
        kcal=quantize_decimal(preset.kcal * multiplier),
        protein_g=quantize_decimal(preset.protein_g * multiplier),
        fat_g=quantize_decimal(preset.fat_g * multiplier),
        carb_g=quantize_decimal(preset.carb_g * multiplier),
    )
